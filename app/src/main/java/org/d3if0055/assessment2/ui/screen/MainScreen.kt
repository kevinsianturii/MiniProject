...
import org.d3if0055.assessment2.navigation.Screen
import org.d3if0055.assessment2.ui.theme.Assessment2Theme
import org.d3if0055.assessment2.util.SettingsDataStore
    ...
    if (data.isEmpty()) {
        Column (
            ...
            Icon(
                ...
                contentDescription = stringResource(R.string.list_kosong),
        ...
        if (showList) {
            LazyColumn(
                ...
                items(data) {
                    ListItem(resep = it) {
                        navController.navigate(Screen.FormUbah.widthId(it.id))
                    }
                    Divider()
                }
            }
...
fun ListItem(resep: Resep, onClick: () -> Unit) {
    Row(
        ...
        val painter = rememberAsyncImagePainter(model = resep.imageUri)
        Image(
            painter = painter,
